...
if "!ENABLE_GITHUB!"=="1" (
  ...
  git push
  if errorlevel 1 goto fail_after_commit
  gh release create v!NEW_VERSION! "!UPDATE_DIR!\DBYHOME-!NEW_VERSION!-quick.zip" "!UPDATE_DIR!\DBYHOME-!NEW_VERSION!-quick-manifest.json" "!UPDATE_DIR!\DBYHOME-!NEW_VERSION!-standard.zip" "!UPDATE_DIR!\DBYHOME-!NEW_VERSION!-standard-manifest.json" --title "DBY HOME v!NEW_VERSION! (QUICK)" --notes "Quick delta update with standard fallback"
  if errorlevel 1 goto fail_after_commit
)
echo.
