...
const WATCHLIST_KEY = 'dbyfinance-bond-watchlist'
...
function parseBondQuantity(input: string) {
  const parsed = parseLocalizedNumber(input.toLowerCase().replace(/cc/g, '').replace(/ccq/g, ''))
...
function buildSnapshots(holdings: HoldingRecord[]) {
  return holdings
    .filter((holding) => holding.category === 'bonds')
    .map((holding) => {
      const quantity = Math.max(parseBondQuantity(holding.quantity), 0)
      ...
      return {
        symbol: normalizeBondSymbol(holding.symbol),
        name: holding.name,
        quantity,
        averageCost,
        pnlPercent: holding.pnlPercent,
        marketValue,
      } satisfies BondSnapshot
    })
...
function BondChart({ symbol, resolution, chartType }: BondChartProps) {
  ...
  useEffect(() => {
    ...
    // Generate bond data (steady upward trend)
    try {
      const data = generateBondCandles(symbol, resolution, 1000)
      if (data.length === 0) {
...
export function BondTracker({
  ...
  const [watchlist, setWatchlist] = useState<string[]>(() => {
    ...
    return localList.length ? localList : ['VFF', 'TCBF', 'SSIBF', 'VNDBF', 'VLBF']
  })
  
  const [resolution, setResolution] = useState<BondResolution>('D')
  const [chartType, setChartType] = useState<'candlestick' | 'area' | 'line'>('area')
  const [watchInput, setWatchInput] = useState('')
  const [watchError, setWatchError] = useState('')
  const [showSearch, setShowSearch] = useState(false)
  
  const selectedSymbol = selectedSymbolProp || (snapshots[0]?.symbol ?? watchlist[0] ?? 'VFF')
  const setSelectedSymbol = onSelectSymbol || (() => {})
  
  const [tickersCache, setTickersCache] = useState<Record<string, BondTicker>>({})
  ...
  const watchSuggestions = useMemo(() => {
    const query = normalizeBondSymbol(watchInput)
    if (!query) return []
  ...
  const loadAllTickers = async () => {
    ...
    const results: Record<string, BondTicker> = {}
    
    try {
      await Promise.all(
        symbolsToFetch.map(async (sym) => {
          try {
            const tk = await fetchBondTicker(sym)
            results[sym] = tk
          } catch (err) {
  ...
  const handleAddWatchSymbol = async (symbolInput = watchInput) => {
    const normalized = normalizeBondSymbol(symbolInput)
    ...
    const isPredefined = BOND_SUGGESTIONS.some((item) => item.symbol === normalized)
    if (isPredefined) {
    ...
    const isValid = await validateBondSymbol(normalized)
    if (!isValid) {
  ...
  const renderWatchlistRow = (sym: string) => {
    ...
    const suggest = BOND_SUGGESTIONS.find(g => g.symbol === sym)
    const snap = snapshots.find(h => h.symbol === sym)
    const fullName = snap?.name || suggest?.name || 'Trái phiếu'
    ...
    const displayPrice = priceVal > 0 ? new Intl.NumberFormat('vi-VN').format(Math.round(priceVal)) : '-'
          ...
          <div style={{ display: 'flex', alignItems: 'center', minWidth: 0 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', minWidth: 0 }}>
              <div style={{ display: 'flex', alignItems: 'center' }}>
                ...
                <span className="tv-badge-delayed">NAV</span>
    ...
    <section className="gold-tracker-container stock-tracker-container">
      ...
      <div className="tracker-header">
        <div className="tracker-title">
          ...
          <h3>Biểu đồ quỹ trái phiếu trực tuyến</h3>
          <span className="source-tag">Nguồn: VinaCapital / Techcom / Hoạt động NAV tự động</span>
              ...
              <div className="tv-watchlist-search-container" style={{ position: 'relative', padding: '8px 12px', borderBottom: '1px solid var(--color-border)', background: 'var(--color-bg-input)' }}>
                <div style={{ display: 'flex', gap: '6px' }}>
                  <input
                    ...
                    placeholder="Tìm mã quỹ/trái phiếu..."
              ...
              <table className="tv-watchlist-table">
                ...
                <thead>
                  <tr>
                    ...
                    <th className="tv-watchlist-th align-right">Giá NAV</th>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px', flexWrap: 'wrap' }}>
              <div className="chart-timeframe-tabs">
                {[
                  ...
                  <button
                    key={tf.value}
                    onClick={() => setResolution(tf.value as BondResolution)}
